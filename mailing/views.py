...
class MailingCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_form(self, form_class=None):  # отображение только тех контактов, которые созданы пользователем
        ...
        form.fields['customers'].queryset = form.fields['customers'].queryset.filter(owner=self.request.user.id)
        return form
...
class LogListView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):  # отображение логов только тех рассылок, которые созданы пользователем
        ...
        for log in Log.objects.all():
            if log.mailing.owner == self.request.user or self.request.user.is_manager:  # менеджеру доступны все логи
                obj_lst.append(log)
        obj_lst.reverse()
...
class LogDetailView(DetailView):
    ...
    def dispatch(self, request, *args, **kwargs):  # доступ к логу только по рассылке, которая создана пользователем
        obj = self.get_object()
        if (obj.mailing.owner != request.user) and (not self.request.user.is_manager):  # менеджеру доступны все логи
            return redirect('mailing:access_error')
        return super().dispatch(request, *args, **kwargs)
